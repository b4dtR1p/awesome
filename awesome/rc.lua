...
rang        = terminal .. " -name 'ranger' -e ranger"
chrome      = "chromium"
fire        = "firefox"
...
batwidget = lain.widgets.bat({
    ...
    settings = function()
        ...
        elseif bat_now.status == "Charging" then
            widget:set_markup(markup(green, space2 ..  bat_now.perc .. "+ " .. bat_now.time))
            baticon:set_image(beautiful.widget_bat_charge)
        elseif tonumber(bat_now.perc) <= 30 then
            widget:set_markup(markup(red, space2 .. bat_now.perc .. "% " .. bat_now.time))
            baticon:set_image(beautiful.widget_bat_low)
        elseif tonumber(bat_now.perc) <= 50 then
            widget:set_markup(markup(yellow, space2 .. bat_now.perc .. "% " .. bat_now.time))
            baticon:set_image(beautiful.widget_bat_half)
        elseif tonumber(bat_now.perc) <= 90 then
            widget:set_markup(markup(white, space2 ..  bat_now.perc .. "% " .. bat_now.time))
...
yawn = lain.widgets.yawn(714186, {
        settings = function()
                    widget:set_markup(markup("#aaaaaa", forecast:lower() .. " @ " .. units .. "°C "))
end
})
...
netwidget = wibox.widget.background(lain.widgets.net({
        settings = function()
                    widget:set_markup(markup("#7AC82E", " " .. net_now.received) .. " " .. markup("#46A8C3", " " .. net_now.sent .. " ")) end
}), "#313131")
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "d", function () awful.util.spawn("dmenu_run -b -fn 'Source Code Pro-9' -nb '#151515' -nf '#d0d0d0' -sb '#151515' -sf '#ff8c00' -p 'run >> '") end),
     awful.key({ modkey }, "F12", function () awful.util.spawn("sh /home/simone/.lockit.sh") end),
       -- Window switching
...
awful.rules.rules = {
    ...
    -- floatng apps
    { rule_any = { class = {"Gpicview", "mpv", "pinentry"} },
    ...
    { rule = { class = "dwb" },
      properties = { tag = tags[1][1] } },
    { rule = { class = "ncmpcpp", "Mplayer", "mpv" },
